/* ============================================
   Shades of Pain — Reader App (Netlify)
   ============================================ */
...
function buildPagedContent(chapter) {
    ...
    if (currentChunk.trim().length > 0) pagedContentHTML.push(currentChunk);

    if (chapter.audioUrl) {
        const audioHtml = `<div class="chapter-audio"><a href="${chapter.audioUrl}" target="_blank">Слухайте озвучку на Telegram →</a></div>`;
        if (pagedContentHTML.length > 0) pagedContentHTML[pagedContentHTML.length - 1] += audioHtml;
        else pagedContentHTML.push(audioHtml);
    }
...
function renderCharacters(characters) {
    ...
    characters.forEach(c => {
        ...
        `;
        card.addEventListener('click', () => openCharacterPage(c));
...
function setupEventListeners() {
    ...
    els.continueBtn.addEventListener('click', () => { if (savedChapterNum) openChapter(savedChapterNum); });

    els.menuToggle.addEventListener('click', () => els.sidebar.classList.contains('open') ? closeSidebar() : openSidebar());
    els.sidebarClose.addEventListener('click', closeSidebar);
    els.sidebarOverlay.addEventListener('click', closeSidebar);
    els.backToHomeBtn.addEventListener('click', () => { showHero(); closeSidebar(); });
    if (els.characterBackBtn) els.characterBackBtn.addEventListener('click', showHero);

    els.startReadingBtn.addEventListener('click', () => { if (allChapters.length > 0) openChapter(1); });

    els.prevBtn.addEventListener('click', () => { if (currentChapter && currentChapter.number > 1) openChapter(currentChapter.number - 1); });
    ...
    document.addEventListener('touchend', e => {
        ...
        if (dx > 80 && dy < 60 && touchStartX < 30) openSidebar();
        if (dx < -80 && dy < 60 && els.sidebar.classList.contains('open')) closeSidebar();
    }, { passive: true });
}
